  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const { products: fetched } = await medusa.store.product.list({ limit: 8, region_id, fields: "*categories,*images,*tags,+metadata,*variants.options,*variants.options.option" } as any)
