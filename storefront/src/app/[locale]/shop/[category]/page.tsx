...
async function getProductsByCategory(categoryId: string, locale?: string) {
  try {
    ...
    const data = await medusaFetch<{ products: any[] }>(
      `/store/products?limit=200&region_id=${regionId}&category_id[]=${categoryId}&fields=*categories,*images,*tags,+metadata,+variants.inventory_quantity,*variants.options,*variants.options.option`,
      { locale }
    )
